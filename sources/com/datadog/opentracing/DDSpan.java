package com.datadog.opentracing;

import com.datadog.trace.api.DDTags;
import com.datadog.trace.api.interceptor.MutableSpan;
import com.datadog.trace.common.util.Clock;
import java.io.PrintWriter;
import java.io.StringWriter;
import java.lang.ref.WeakReference;
import java.math.BigInteger;
import java.util.HashMap;
import java.util.Map;
import java.util.concurrent.TimeUnit;
import java.util.concurrent.atomic.AtomicLong;
import p218io.opentracing.Span;
import p218io.opentracing.tag.Tag;

public class DDSpan implements MutableSpan, Span {

    /* renamed from: a */
    volatile WeakReference<DDSpan> f4080a;

    /* renamed from: b */
    private final DDSpanContext f4081b;

    /* renamed from: c */
    private final long f4082c;

    /* renamed from: d */
    private final long f4083d;

    /* renamed from: e */
    private final AtomicLong f4084e;

    /* renamed from: f */
    private final LogHandler f4085f;

    DDSpan(long j, DDSpanContext dDSpanContext) {
        this(j, dDSpanContext, new DefaultLogHandler());
    }

    DDSpan(long j, DDSpanContext dDSpanContext, LogHandler logHandler) {
        this.f4084e = new AtomicLong();
        this.f4081b = dDSpanContext;
        this.f4085f = logHandler;
        if (j <= 0) {
            this.f4082c = Clock.currentMicroTime();
            this.f4083d = dDSpanContext.getTrace().getCurrentTimeNano();
        } else {
            this.f4082c = j;
            this.f4083d = 0;
        }
        dDSpanContext.getTrace().registerSpan(this);
    }

    public boolean isFinished() {
        return this.f4084e.get() != 0;
    }

    /* renamed from: a */
    private void m2554a(long j) {
        if (this.f4084e.compareAndSet(0, Math.max(1, j))) {
            this.f4081b.getTrace().addSpan(this);
        }
    }

    public final void finish() {
        if (this.f4083d > 0) {
            m2554a(this.f4081b.getTrace().getCurrentTimeNano() - this.f4083d);
        } else {
            finish(Clock.currentMicroTime());
        }
    }

    public final void finish(long j) {
        m2554a(TimeUnit.MICROSECONDS.toNanos(j - this.f4082c));
    }

    public DDSpan setError(boolean z) {
        this.f4081b.setErrorFlag(z);
        return this;
    }

    public final void drop() {
        this.f4081b.getTrace().dropSpan(this);
    }

    public final boolean isRootSpan() {
        return BigInteger.ZERO.equals(this.f4081b.getParentId());
    }

    @Deprecated
    public MutableSpan getRootSpan() {
        return getLocalRootSpan();
    }

    public MutableSpan getLocalRootSpan() {
        return context().getTrace().getRootSpan();
    }

    public void setErrorMeta(Throwable th) {
        setError(true);
        setTag(DDTags.ERROR_MSG, th.getMessage());
        setTag(DDTags.ERROR_TYPE, th.getClass().getName());
        StringWriter stringWriter = new StringWriter();
        th.printStackTrace(new PrintWriter(stringWriter));
        setTag("error.stack", stringWriter.toString());
    }

    public final DDSpan setTag(String str, String str2) {
        context().setTag(str, str2);
        return this;
    }

    public final DDSpan setTag(String str, boolean z) {
        context().setTag(str, Boolean.valueOf(z));
        return this;
    }

    public final DDSpan setTag(String str, Number number) {
        context().setTag(str, number);
        return this;
    }

    public <T> Span setTag(Tag<T> tag, T t) {
        context().setTag(tag.getKey(), t);
        return this;
    }

    public final DDSpanContext context() {
        return this.f4081b;
    }

    public final String getBaggageItem(String str) {
        return this.f4081b.getBaggageItem(str);
    }

    public final DDSpan setBaggageItem(String str, String str2) {
        this.f4081b.setBaggageItem(str, str2);
        return this;
    }

    public final DDSpan setOperationName(String str) {
        context().setOperationName(str);
        return this;
    }

    public final DDSpan log(Map<String, ?> map) {
        this.f4085f.log(map, this);
        return this;
    }

    public final DDSpan log(long j, Map<String, ?> map) {
        this.f4085f.log(j, map, this);
        return this;
    }

    public final DDSpan log(String str) {
        this.f4085f.log(str, this);
        return this;
    }

    public final DDSpan log(long j, String str) {
        this.f4085f.log(j, str, this);
        return this;
    }

    public final DDSpan setServiceName(String str) {
        context().setServiceName(str);
        return this;
    }

    public final DDSpan setResourceName(String str) {
        context().setResourceName(str);
        return this;
    }

    public final DDSpan setSamplingPriority(int i) {
        context().setSamplingPriority(i);
        return this;
    }

    public final DDSpan setSpanType(String str) {
        context().setSpanType(str);
        return this;
    }

    public Map<String, String> getMeta() {
        HashMap hashMap = new HashMap();
        for (Map.Entry next : context().getBaggageItems().entrySet()) {
            hashMap.put((String) next.getKey(), (String) next.getValue());
        }
        for (Map.Entry next2 : getTags().entrySet()) {
            hashMap.put((String) next2.getKey(), String.valueOf(next2.getValue()));
        }
        return hashMap;
    }

    public Map<String, Number> getMetrics() {
        return this.f4081b.getMetrics();
    }

    public long getStartTime() {
        long j = this.f4083d;
        return j > 0 ? j : TimeUnit.MICROSECONDS.toNanos(this.f4082c);
    }

    public long getDurationNano() {
        return this.f4084e.get();
    }

    public String getServiceName() {
        return this.f4081b.getServiceName();
    }

    public BigInteger getTraceId() {
        return this.f4081b.getTraceId();
    }

    public BigInteger getSpanId() {
        return this.f4081b.getSpanId();
    }

    public BigInteger getParentId() {
        return this.f4081b.getParentId();
    }

    public String getResourceName() {
        return this.f4081b.getResourceName();
    }

    public String getOperationName() {
        return this.f4081b.getOperationName();
    }

    public Integer getSamplingPriority() {
        int samplingPriority = this.f4081b.getSamplingPriority();
        if (samplingPriority == Integer.MIN_VALUE) {
            return null;
        }
        return Integer.valueOf(samplingPriority);
    }

    public String getSpanType() {
        return this.f4081b.getSpanType();
    }

    public Map<String, Object> getTags() {
        return context().getTags();
    }

    public String getType() {
        return this.f4081b.getSpanType();
    }

    public Boolean isError() {
        return Boolean.valueOf(this.f4081b.getErrorFlag());
    }

    public int getError() {
        return this.f4081b.getErrorFlag() ? 1 : 0;
    }

    public String toString() {
        return this.f4081b.toString() + ", duration_ns=" + this.f4084e;
    }
}
