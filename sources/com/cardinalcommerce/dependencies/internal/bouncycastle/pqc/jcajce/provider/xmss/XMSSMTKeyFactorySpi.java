...
import com.cardinalcommerce.p060a.setDrawingCacheBackgroundColor;
import com.cardinalcommerce.p060a.setLayerPaint;
...
public class XMSSMTKeyFactorySpi extends KeyFactorySpi implements AsymmetricKeyInfoConverter {
    public final PrivateKey cca_continue(PrivateKeyInfo privateKeyInfo) throws IOException {
        return new setLayerPaint(privateKeyInfo);
    }

    public final KeySpec engineGetKeySpec(Key key, Class cls) throws InvalidKeySpecException {
        if (key instanceof setLayerPaint) {
            if (PKCS8EncodedKeySpec.class.isAssignableFrom(cls)) {
                return new PKCS8EncodedKeySpec(key.getEncoded());
            }
        } else if (!(key instanceof setDrawingCacheBackgroundColor)) {
    ...
    public final Key engineTranslateKey(Key key) throws InvalidKeyException {
        if ((key instanceof setLayerPaint) || (key instanceof setDrawingCacheBackgroundColor)) {
            return key;
        }
    ...
    public final PublicKey getInstance(SubjectPublicKeyInfo subjectPublicKeyInfo) throws IOException {
        return new setDrawingCacheBackgroundColor(subjectPublicKeyInfo);
    }

    public PrivateKey engineGeneratePrivate(KeySpec keySpec) throws InvalidKeySpecException {
        if (keySpec instanceof PKCS8EncodedKeySpec) {
            try {
                return new setLayerPaint(PrivateKeyInfo.init(setLocationDataConsentGiven.getInstance(((PKCS8EncodedKeySpec) keySpec).getEncoded())));
            } catch (Exception e) {
    ...
    public PublicKey engineGeneratePublic(KeySpec keySpec) throws InvalidKeySpecException {
        if (keySpec instanceof X509EncodedKeySpec) {
            try {
                return new setDrawingCacheBackgroundColor(SubjectPublicKeyInfo.init(((X509EncodedKeySpec) keySpec).getEncoded()));
            } catch (Exception e) {
                throw new InvalidKeySpecException(e.toString());
            }
        } else {
