...
final class Ed25519Constants {
    ...
    /* renamed from: B2 */
    static final Ed25519.CachedXYT[] f56613B2 = new Ed25519.CachedXYT[8];
    static final Ed25519.CachedXYT[][] B_TABLE = ((Ed25519.CachedXYT[][]) Array.newInstance(Ed25519.CachedXYT.class, new int[]{32, 8}));

    /* renamed from: D */
    static final long[] f56614D = Field25519.expand(toLittleEndian(D_BI));

    /* renamed from: D2 */
    static final long[] f56615D2 = Field25519.expand(toLittleEndian(D2_BI));
    ...
    static {
        ...
        BigInteger unused = point.f56617y = BigInteger.valueOf(4).multiply(BigInteger.valueOf(5).modInverse(P_BI)).mod(P_BI);
        BigInteger unused2 = point.f56616x = recoverX(point.f56617y);
        Point point2 = point;
        for (int i = 0; i < 32; i++) {
            Point point3 = point2;
        ...
        for (int i4 = 0; i4 < 8; i4++) {
            f56613B2[i4] = getCachedXYT(point);
    ...
    private static class Point {
        ...
        /* renamed from: x */
        public BigInteger f56616x;
        /* access modifiers changed from: private */

        /* renamed from: y */
        public BigInteger f56617y;
    ...
    private static Point edwards(Point point, Point point2) {
        ...
        BigInteger mod = D_BI.multiply(point.f56616x.multiply(point2.f56616x).multiply(point.f56617y).multiply(point2.f56617y)).mod(P_BI);
        BigInteger unused = point3.f56616x = point.f56616x.multiply(point2.f56617y).add(point2.f56616x.multiply(point.f56617y)).multiply(BigInteger.ONE.add(mod).modInverse(P_BI)).mod(P_BI);
        BigInteger unused2 = point3.f56617y = point.f56617y.multiply(point2.f56617y).add(point.f56616x.multiply(point2.f56616x)).multiply(BigInteger.ONE.subtract(mod).modInverse(P_BI)).mod(P_BI);
        return point3;
    }
    ...
    private static Ed25519.CachedXYT getCachedXYT(Point point) {
        return new Ed25519.CachedXYT(Field25519.expand(toLittleEndian(point.f56617y.add(point.f56616x).mod(P_BI))), Field25519.expand(toLittleEndian(point.f56617y.subtract(point.f56616x).mod(P_BI))), Field25519.expand(toLittleEndian(D2_BI.multiply(point.f56616x).multiply(point.f56617y).mod(P_BI))));
    }
}
