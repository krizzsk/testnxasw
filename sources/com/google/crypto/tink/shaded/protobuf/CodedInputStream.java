...
public abstract class CodedInputStream {
    ...
    private static final class UnsafeDirectNioDecoder extends CodedInputStream {
            ...
            if (this.limit != j4) {
                ...
                if (b2 >= 0) {
                    ...
                    if (b3 < 0) {
                        ...
                        long j7 = j6 + 1;
                        byte b4 = b3 ^ (UnsafeUtil.getByte(j6) << Ascii.f55149SO);
                        if (b4 >= 0) {
                            j = (long) (b4 ^ 16256);
                        } else {
                            j6 = j7 + 1;
        ...
        public int readRawLittleEndian32() throws IOException {
            ...
            if (this.limit - j >= 4) {
                this.pos = 4 + j;
                return ((UnsafeUtil.getByte(j + 3) & 255) << Ascii.CAN) | (UnsafeUtil.getByte(j) & 255) | ((UnsafeUtil.getByte(1 + j) & 255) << 8) | ((UnsafeUtil.getByte(2 + j) & 255) << 16);
            }
            throw InvalidProtocolBufferException.truncatedMessage();
        }
