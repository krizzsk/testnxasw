...
class DisplayNotification {
    ...
    private void showNotification(CommonNotificationBuilder.DisplayNotificationInfo displayNotificationInfo) {
        if (Log.isLoggable(Constants.TAG, 3)) {
            SystemUtils.log(3, Constants.TAG, "Showing notification", (Throwable) null, "com.google.firebase.messaging.DisplayNotification", 2);
        }
        ((NotificationManager) this.context.getSystemService("notification")).notify(displayNotificationInfo.tag, displayNotificationInfo.f56624id, displayNotificationInfo.notificationBuilder.build());
    }
    ...
    private void waitForAndApplyImageDownload(NotificationCompat.Builder builder, ImageDownload imageDownload) {
        if (imageDownload != null) {
            try {
                ...
                sb.append(valueOf);
                SystemUtils.log(5, Constants.TAG, sb.toString(), (Throwable) null, "com.google.firebase.messaging.DisplayNotification", 9);
            } catch (InterruptedException unused) {
                SystemUtils.log(5, Constants.TAG, "Interrupted while downloading image, showing notification without it", (Throwable) null, "com.google.firebase.messaging.DisplayNotification", 6);
                imageDownload.close();
                Thread.currentThread().interrupt();
            } catch (TimeoutException unused2) {
                SystemUtils.log(5, Constants.TAG, "Failed to download image in time, showing notification without it", (Throwable) null, "com.google.firebase.messaging.DisplayNotification", 4);
                imageDownload.close();
            }
        }
    }
