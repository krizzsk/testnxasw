...
public final class zzku extends zzkw {
    private final AlarmManager zza = ((AlarmManager) this.zzs.zzav().getSystemService(NotificationCompat.CATEGORY_ALARM));
    private zzao zzb;
    private Integer zzc;

    protected zzku(zzli zzli) {
        super(zzli);
    }
    ...
    public final void zzd(long j) {
        ...
        this.zzs.zzf();
        if (j < Math.max(0, ((Long) zzel.zzw.zza((Object) null)).longValue()) && !zzi().zze()) {
            zzi().zzd(j);
        }
        this.zzs.zzax();
        if (Build.VERSION.SDK_INT >= 24) {
            ...
            zzbt.zza(zzav2, new JobInfo.Builder(zzf, componentName).setMinimumLatency(j).setOverrideDeadline(j + j).setExtras(persistableBundle).build(), "com.google.android.gms", "UploadAlarm");
            return;
        }
        AlarmManager alarmManager = this.zza;
        if (alarmManager != null) {
            this.zzs.zzf();
            alarmManager.setInexactRepeating(2, elapsedRealtime, Math.max(((Long) zzel.zzr.zza((Object) null)).longValue(), j), zzh());
        }
    }
}
