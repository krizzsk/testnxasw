        ...
        while (i2 < size) {
            try {
                if (!cursorWindow.allocRow()) {
                    SystemUtils.log(3, "DataHolder", "Allocating additional cursor window for large data set (row " + i2 + ")", (Throwable) null, "com.google.android.gms.common.data.DataHolder", 9);
                    cursorWindow = new CursorWindow(false);
                    cursorWindow.setStartPosition(i2);
                    cursorWindow.setNumColumns(builder.zaa.length);
                    arrayList.add(cursorWindow);
                    if (!cursorWindow.allocRow()) {
                        SystemUtils.log(6, "DataHolder", "Unable to allocate row to hold data.", (Throwable) null, "com.google.android.gms.common.data.DataHolder", 44);
                ...
                if (!z) {
                    SystemUtils.log(3, "DataHolder", "Couldn't populate window data for row " + i2 + " - allocating new window.", (Throwable) null, "com.google.android.gms.common.data.DataHolder", 35);
    ...
    public final void finalize() throws Throwable {
        try {
            if (this.zak && this.zah.length > 0 && !isClosed()) {
                close();
                String obj = toString();
                SystemUtils.log(6, "DataBuffer", "Internal data leak within a DataBuffer object detected!  Be sure to explicitly call release() on all DataBuffer extending objects when you are done with them. (internal object: " + obj + ")", (Throwable) null, "com.google.android.gms.common.data.DataHolder", 4);
            }
        } finally {
            super.finalize();
        }
