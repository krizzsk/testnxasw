...
public abstract class CustomViewTarget<T extends View, Z> implements Target<Z> {
    ...
    private static final int VIEW_TAG_ID = 2131431087;
    ...
    static final class SizeDeterminer {
        ...
        private int getTargetDimen(int i, int i2, int i3) {
            ...
            if (Log.isLoggable(CustomViewTarget.TAG, 4)) {
                SystemUtils.log(4, CustomViewTarget.TAG, "Glide treats LayoutParams.WRAP_CONTENT as a request for an image the size of this device's screen dimensions. If you want to load the original image and are ok with the corresponding memory cost and OOMs (depending on the input size), use .override(Target.SIZE_ORIGINAL). Otherwise, use LayoutParams.MATCH_PARENT, set layout_width and layout_height to fixed dimension, or use .override() with fixed dimensions.", (Throwable) null, "com.bumptech.glide.request.target.CustomViewTarget$SizeDeterminer", 451);
            }
        ...
        private static final class SizeDeterminerLayoutListener implements ViewTreeObserver.OnPreDrawListener {
            ...
            public boolean onPreDraw() {
                if (Log.isLoggable(CustomViewTarget.TAG, 2)) {
                    SystemUtils.log(2, CustomViewTarget.TAG, "OnGlobalLayoutListener called attachStateListener=" + this, (Throwable) null, "com.bumptech.glide.request.target.CustomViewTarget$SizeDeterminer$SizeDeterminerLayoutListener", 484);
                }
