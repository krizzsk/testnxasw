...
public class WebpDecoder implements GifDecoder {
    ...
    public WebpDecoder(GifDecoder.BitmapProvider bitmapProvider, WebpImage webpImage, ByteBuffer byteBuffer, int i, WebpFrameCacheStrategy webpFrameCacheStrategy) {
        ...
        for (int i3 = 0; i3 < this.mWebPImage.getFrameCount(); i3++) {
            ...
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "mFrameInfos: " + this.mFrameInfos[i3].toString(), (Throwable) null, "com.bumptech.glide.integration.webp.decoder.WebpDecoder", 68);
            }
        }
    ...
    public Bitmap getNextFrame() {
        ...
        if (this.mCacheStrategy.noCache() || (bitmap = this.mFrameBitmapCache.get(Integer.valueOf(currentFrameIndex))) == null) {
            ...
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "frameNumber=" + currentFrameIndex + ", nextIndex=" + prepareCanvasWithBlending, (Throwable) null, "com.bumptech.glide.integration.webp.decoder.WebpDecoder", 228);
            }
            while (prepareCanvasWithBlending < currentFrameIndex) {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "renderFrame, index=" + prepareCanvasWithBlending + ", blend=" + webpFrameInfo.blendPreviousFrame + ", dispose=" + webpFrameInfo.disposeBackgroundColor, (Throwable) null, "com.bumptech.glide.integration.webp.decoder.WebpDecoder", 241);
                }
            ...
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "renderFrame, index=" + currentFrameIndex + ", blend=" + webpFrameInfo2.blendPreviousFrame + ", dispose=" + webpFrameInfo2.disposeBackgroundColor, (Throwable) null, "com.bumptech.glide.integration.webp.decoder.WebpDecoder", 259);
            }
            cacheFrameBitmap(currentFrameIndex, obtain);
            return obtain;
        }
        if (Log.isLoggable(TAG, 3)) {
            SystemUtils.log(3, TAG, "hit frame bitmap from memory cache, frameNumber=" + currentFrameIndex, (Throwable) null, "com.bumptech.glide.integration.webp.decoder.WebpDecoder", 210);
        }
    ...
    private void renderFrame(int i, Canvas canvas) {
        ...
        if (i2 != 0 && i3 != 0) {
            ...
            try {
                ...
                SystemUtils.log(6, TAG, "Rendering of frame failed. Frame number: " + i, (Throwable) null, "com.bumptech.glide.integration.webp.decoder.WebpDecoder", 289);
            } catch (Throwable th) {
                frame.dispose();
                throw th;
            }
