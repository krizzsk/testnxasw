...
public class HttpUrlFetcher implements DataFetcher<InputStream> {
    ...
    public void loadData(Priority priority, DataFetcher.DataCallback<? super InputStream> dataCallback) {
        ...
        try {
            ...
            if (Log.isLoggable(TAG, 2)) {
                ...
                SystemUtils.log(2, TAG, sb.toString(), (Throwable) null, "com.bumptech.glide.load.data.HttpUrlFetcher", 67);
            }
        } catch (IOException e) {
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "Failed to load data for url", e, "com.bumptech.glide.load.data.HttpUrlFetcher", 62);
            }
            ...
            if (Log.isLoggable(TAG, 2)) {
                SystemUtils.log(2, TAG, "Finished http url fetcher fetch in " + LogTime.getElapsedMillis(logTime), (Throwable) null, "com.bumptech.glide.load.data.HttpUrlFetcher", 67);
            }
            throw th;
        }
    }
    ...
    private static int getHttpStatusCodeOrInvalid(HttpURLConnection httpURLConnection) {
        try {
            ...
            SystemUtils.log(3, TAG, "Failed to get a response code", e, "com.bumptech.glide.load.data.HttpUrlFetcher", 139);
            return -1;
        }
    }
    ...
    private InputStream getStreamForSuccessfulRequest(HttpURLConnection httpURLConnection) throws HttpException {
        try {
            if (TextUtils.isEmpty(httpURLConnection.getContentEncoding())) {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "Got non empty content encoding: " + httpURLConnection.getContentEncoding(), (Throwable) null, "com.bumptech.glide.load.data.HttpUrlFetcher", 184);
                }
