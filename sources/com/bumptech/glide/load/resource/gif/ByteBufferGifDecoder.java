...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    private GifDrawableResource decode(ByteBuffer byteBuffer, int i, int i2, GifHeaderParser gifHeaderParser, Options options) {
        ...
        try {
                ...
                if (parseHeader.getStatus() == 0) {
                    ...
                    if (nextFrame == null) {
                        if (Log.isLoggable(TAG, 2)) {
                            SystemUtils.log(2, TAG, "Decoded GIF from stream in " + LogTime.getElapsedMillis(logTime), (Throwable) null, "com.bumptech.glide.load.resource.gif.ByteBufferGifDecoder", 129);
                        }
                        return null;
                    }
            ...
            if (Log.isLoggable(TAG, 2)) {
                str = "com.bumptech.glide.load.resource.gif.ByteBufferGifDecoder";
                SystemUtils.log(2, TAG, "Decoded GIF from stream in " + LogTime.getElapsedMillis(logTime), (Throwable) null, str, 129);
            }
        }
    }
    ...
    private static int getSampleSize(GifHeader gifHeader, int i, int i2) {
        ...
        if (Log.isLoggable(TAG, 2) && max > 1) {
            SystemUtils.log(2, TAG, "Downsampling GIF, sampleSize: " + max + ", target dimens: [" + i + "x" + i2 + "], actual dimens: [" + gifHeader.getWidth() + "x" + gifHeader.getHeight() + Const.jaRight, (Throwable) null, "com.bumptech.glide.load.resource.gif.ByteBufferGifDecoder", 142);
        }
        return max;
    }
    ...
    static class GifHeaderParserPool {
        private final Queue<GifHeaderParser> pool = C1850Util.createQueue(0);
