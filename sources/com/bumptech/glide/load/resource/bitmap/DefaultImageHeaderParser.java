...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static boolean handles(int i) {
        return (i & EXIF_MAGIC_NUMBER) == EXIF_MAGIC_NUMBER || i == 19789 || i == 18761;
    }
        ...
        try {
            ...
            if (!handles(uInt16)) {
                if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "Parser doesn't handle magic number: " + uInt16, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 223);
                }
                return -1;
            }
            ...
            if (moveToExifSegmentAndGetLength == -1) {
                if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "Failed to parse exif segment length, or exif segment not found", (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 230);
                }
                return -1;
            }
    ...
    private int parseExifSegment(Reader reader, byte[] bArr, int i) throws IOException {
        ...
        if (read != i) {
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "Unable to read exif segment data, length: " + i + ", actually read: " + read, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 256);
            }
            return -1;
            ...
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "Missing jpeg exif preamble", (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 272);
            }
            return -1;
        }
    }
    ...
    private int moveToExifSegmentAndGetLength(Reader reader) throws IOException {
        ...
        do {
            ...
            if (uInt82 != 255) {
                if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "Unknown segmentId=" + uInt82, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 301);
                }
                return -1;
            }
            ...
            if (uInt8 == 217) {
                if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "Found MARKER_EOI in exif segment", (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 311);
                }
                return -1;
            }
        ...
        if (Log.isLoggable(TAG, 3)) {
            SystemUtils.log(3, TAG, "Unable to skip enough data, type: " + uInt8 + ", wanted to skip: " + uInt16 + ", but actually skipped: " + skip, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 323);
        }
        return -1;
    }
    ...
    private static int parseExifSegment(RandomAccessReader randomAccessReader) {
        ...
        short int16 = randomAccessReader2.getInt16(6);
        if (int16 == 18761) {
            byteOrder = ByteOrder.LITTLE_ENDIAN;
        } else if (int16 != 19789) {
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "Unknown endianness = " + int16, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 355);
            }
        ...
        int int32 = randomAccessReader2.getInt32(10) + 6;
        short int162 = randomAccessReader2.getInt16(int32);
        for (int i = 0; i < int162; i++) {
            int calcTagOffset = calcTagOffset(int32, i);
            short int163 = randomAccessReader2.getInt16(calcTagOffset);
            if (int163 == 274) {
                short int164 = randomAccessReader2.getInt16(calcTagOffset + 2);
                if (int164 >= 1 && int164 <= 12) {
                    int int322 = randomAccessReader2.getInt32(calcTagOffset + 4);
                    if (int322 >= 0) {
                        if (Log.isLoggable(TAG, 3)) {
                            SystemUtils.log(3, TAG, "Got tagIndex=" + i + " tagType=" + int163 + " formatCode=" + int164 + " componentCount=" + int322, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 392);
                        }
                        ...
                        if (i2 <= 4) {
                            ...
                            if (i3 < 0 || i3 > randomAccessReader.length()) {
                                if (Log.isLoggable(TAG, 3)) {
                                    SystemUtils.log(3, TAG, "Illegal tagValueOffset=" + i3 + " tagType=" + int163, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 415);
                                }
                            } else if (i2 >= 0 && i2 + i3 <= randomAccessReader.length()) {
                                return randomAccessReader2.getInt16(i3);
                            } else {
                                if (Log.isLoggable(TAG, 3)) {
                                    SystemUtils.log(3, TAG, "Illegal number of bytes for TI tag data tagType=" + int163, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 422);
                                }
                            }
                        } else if (Log.isLoggable(TAG, 3)) {
                            SystemUtils.log(3, TAG, "Got byte count > 4, not orientation, continuing, formatCode=" + int164, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 407);
                        }
                    } else if (Log.isLoggable(TAG, 3)) {
                        SystemUtils.log(3, TAG, "Negative tiff component count", (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 386);
                    }
                } else if (Log.isLoggable(TAG, 3)) {
                    SystemUtils.log(3, TAG, "Got invalid format code = " + int164, (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser", 378);
                }
            }
        }
        return -1;
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f2168is;

        StreamReader(InputStream inputStream) {
            this.f2168is = inputStream;
        }

        public short getUInt8() throws IOException {
            int read = this.f2168is.read();
            if (read != -1) {
                return (short) read;
            }
            throw new Reader.EndOfFileException();
        ...
        public int read(byte[] bArr, int i) throws IOException {
            int i2 = 0;
            int i3 = 0;
            while (i2 < i && (i3 = this.f2168is.read(bArr, i2, i - i2)) != -1) {
                i2 += i3;
            }
            if (i2 != 0 || i3 != -1) {
                return i2;
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long skip = this.f2168is.skip(j2);
                if (skip <= 0) {
                    if (this.f2168is.read() == -1) {
                        break;
                    }
                    skip = 1;
                }
