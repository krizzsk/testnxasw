    ...
    private static Bitmap drawToBitmap(BitmapPool bitmapPool, Drawable drawable, int i, int i2) {
        if (i == Integer.MIN_VALUE && drawable.getIntrinsicWidth() <= 0) {
            if (Log.isLoggable(TAG, 5)) {
                SystemUtils.log(5, TAG, "Unable to draw " + drawable + " to Bitmap with Target.SIZE_ORIGINAL because the Drawable has no intrinsic width", (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DrawableToBitmapConverter", 53);
            }
            return null;
            ...
            if (Log.isLoggable(TAG, 5)) {
                SystemUtils.log(5, TAG, "Unable to draw " + drawable + " to Bitmap with Target.SIZE_ORIGINAL because the Drawable has no intrinsic height", (Throwable) null, "com.bumptech.glide.load.resource.bitmap.DrawableToBitmapConverter", 64);
            }
            return null;
        }
    }
