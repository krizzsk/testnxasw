...
final class SizeStrategy implements LruPoolStrategy {
    ...
    public void put(Bitmap bitmap) {
        Key key = this.keyPool.get(C1850Util.getBitmapByteSize(bitmap));
    ...
    public Bitmap get(int i, int i2, Bitmap.Config config) {
        int bitmapByteSize = C1850Util.getBitmapByteSize(i, i2, config);
    ...
    public Bitmap removeLast() {
        ...
        if (removeLast != null) {
            decrementBitmapOfSize(Integer.valueOf(C1850Util.getBitmapByteSize(removeLast)));
        }
        return removeLast;
    }
    ...
    public String logBitmap(int i, int i2, Bitmap.Config config) {
        return getBitmapString(C1850Util.getBitmapByteSize(i, i2, config));
    }
    ...
    private static String getBitmapString(Bitmap bitmap) {
        return getBitmapString(C1850Util.getBitmapByteSize(bitmap));
    }

    static String getBitmapString(int i) {
        return Const.jaLeft + i + Const.jaRight;
    }
