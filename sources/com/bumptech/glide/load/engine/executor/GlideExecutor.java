        ...
        public static final UncaughtThrowableStrategy LOG = new UncaughtThrowableStrategy() {
            public void handle(Throwable th) {
                if (th != null && Log.isLoggable(GlideExecutor.TAG, 6)) {
                    SystemUtils.log(6, GlideExecutor.TAG, "Request threw uncaught throwable", th, "com.bumptech.glide.load.engine.executor.GlideExecutor$UncaughtThrowableStrategy$2", 337);
                }
            }
        };
    ...
    private static final class DefaultPriorityThreadFactory implements ThreadFactory {
        ...
        public Thread newThread(Runnable runnable) {
            return new Thread(runnable) {
                public void run() {
                    SystemUtils.setProcessThreadPriority(9);
                    super.run();
                }
            };
        }
