...
public class GifHeaderParser {
    ...
    private void readBitmap() {
        this.header.currentFrame.f2150ix = readShort();
        this.header.currentFrame.f2151iy = readShort();
        this.header.currentFrame.f2149iw = readShort();
        this.header.currentFrame.f2148ih = readShort();
        int read = read();
        boolean z = false;
    ...
    private void readHeader() {
        ...
        if (!sb.toString().startsWith(TunaView.GRAPHICSTYPE_GIF)) {
            this.header.status = 1;
            return;
        }
        readLSD();
    ...
    private int[] readColorTable(int i) {
        ...
        try {
            ...
            if (Log.isLoggable(TAG, 3)) {
                SystemUtils.log(3, TAG, "Format Error Reading Color Table", bufferUnderflowException, "com.bumptech.glide.gifdecoder.GifHeaderParser", 436);
            }
            this.header.status = 1;
        }
        return iArr;
        ...
        if (read > 0) {
            ...
            while (i < this.blockSize) {
                try {
                    ...
                    Exception exc = e;
                    if (Log.isLoggable(TAG, 3)) {
                        SystemUtils.log(3, TAG, "Error Reading Block n: " + i + " count: " + i2 + " blockSize: " + this.blockSize, exc, "com.bumptech.glide.gifdecoder.GifHeaderParser", 483);
                    }
                    this.header.status = 1;
                    return;
                }
