...
import com.didi.map.global.component.departure.fence.FencePolygon;
...
public class FenceUtils {
    ...
    /* renamed from: a */
    private static final float f27566a = 14.0f;

    /* renamed from: b */
    private static final float f27567b = 18.0f;
    ...
    public static boolean positionIsInFence(Map map, FenceInfo fenceInfo, LatLng latLng) {
        if (!(fenceInfo == null || latLng == null || CollectionUtil.isEmpty((Collection<?>) fenceInfo.polygon))) {
            ArrayList arrayList = new ArrayList(fenceInfo.polygon.size());
            for (FenceLatLng next : fenceInfo.polygon) {
                arrayList.add(new LatLng(next.lat, next.lng));
            }
            if (map == null || !PolygonUtil.contains(map, (List<LatLng>) arrayList, latLng)) {
                return false;
            }
            return true;
        }
