...
public class DDSphericalUtil {
    ...
    public static LatLng interpolate(LatLng latLng, LatLng latLng2, double d) {
        ...
        double a = m10770a(latLng, latLng2);
    ...
    /* renamed from: a */
    private static double m10769a(double d, double d2, double d3, double d4) {
        return DDMathUtil.arcHav(DDMathUtil.havDistance(d, d3, d2 - d4));
    }

    /* renamed from: a */
    static double m10770a(LatLng latLng, LatLng latLng2) {
        return m10769a(Math.toRadians(latLng.latitude), Math.toRadians(latLng.longitude), Math.toRadians(latLng2.latitude), Math.toRadians(latLng2.longitude));
    }

    public static double computeAngleBetween(double d, double d2, double d3, double d4) {
        return m10769a(Math.toRadians(d), Math.toRadians(d2), Math.toRadians(d3), Math.toRadians(d4));
    }

    public static double computeDistanceBetween(LatLng latLng, LatLng latLng2) {
        return m10770a(latLng, latLng2) * 6371009.0d;
    }
    ...
    public static double computeLength(List<LatLng> list) {
        ...
        for (LatLng next : list) {
            ...
            d += m10769a(d2, d3, radians3, radians4);
            d2 = radians3;
            d3 = radians4;
        }
        return d * 6371009.0d;
    ...
    public static double computeSignedArea(List<LatLng> list) {
        return m10771a(list, 6371009.0d);
    }

    /* renamed from: a */
    static double m10771a(List<LatLng> list, double d) {
        int size = list.size();
        double d2 = 0.0d;
        if (size < 3) {
            return 0.0d;
        ...
        for (LatLng next : list) {
            ...
            d2 += m10772b(tan2, radians2, d3, d4);
            d3 = tan2;
            d4 = radians2;
        }
        return d2 * d * d;
    ...
    /* renamed from: b */
    private static double m10772b(double d, double d2, double d3, double d4) {
        double d5 = d2 - d4;
        double d6 = d * d3;
