...
@ComponentType(name = "nav_to_edit_account")
public class MexicoNav2UserInfoComponent extends AbsNav2UserInfoComponent {
    public MexicoNav2UserInfoComponent(BaseBusinessContext baseBusinessContext, SidebarItem sidebarItem, String str) {
    ...
    public void setName(UserInfo userInfo) {
        if (userInfo != null) {
            ...
            logger.info("MexicoNav2UserInfoComponent first name  = " + userInfo.getFirst_name() + "  last name = " + userInfo.getLast_name(), new Object[0]);
            if (!TextUtil.isEmpty(userInfo.getLast_name()) && !TextUtils.isEmpty(userInfo.getFirst_name())) {
                ((Nav2UserInfoComponentNewView) this.componentView).setName(userInfo.getFirst_name() + " " + userInfo.getLast_name());
