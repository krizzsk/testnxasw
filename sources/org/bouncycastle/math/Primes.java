    ...
    private static STOutput implSTRandomPrime(Digest digest, int i, byte[] bArr) {
        ...
        if (i2 < 33) {
            ...
            do {
                ...
                inc(bArr2, 1);
                i4++;
                long extract32 = ((long) (((extract32(bArr3) ^ extract32(bArr4)) & (-1 >>> (32 - i2))) | (1 << (i2 - 1)) | 1)) & InternalZipConstants.ZIP_64_SIZE_LIMIT;
        ...
        while (true) {
            if (add.bitLength() > i2) {
                shiftLeft2 = ONE.shiftLeft(i5).subtract(ONE).divide(shiftLeft).add(ONE).shiftLeft(i3);
