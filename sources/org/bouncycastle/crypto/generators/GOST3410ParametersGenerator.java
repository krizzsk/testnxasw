    ...
    private int procedure_A(int i, int i2, BigInteger[] bigIntegerArr, int i3) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                int i16 = i10 + 1;
                BigInteger add = TWO.pow(iArr[i10] - 1).divide(bigIntegerArr4[i16]).add(TWO.pow(iArr[i10] - 1).multiply(bigInteger7).divide(bigIntegerArr4[i16].multiply(TWO.pow(i12 * 16))));
    ...
    private long procedure_Aa(long j, long j2, BigInteger[] bigIntegerArr, int i) {
        ...
        while (true) {
            ...
            while (true) {
                ...
                int i12 = i5 + 1;
                BigInteger add = TWO.pow(iArr[i5] - 1).divide(bigIntegerArr4[i12]).add(TWO.pow(iArr[i5] - 1).multiply(bigInteger7).divide(bigIntegerArr4[i12].multiply(TWO.pow(i8 * 32))));
                if (add.mod(TWO).compareTo(ONE) == 0) {
                    add = add.add(ONE);
                }
                int i13 = 0;
