...
public class KDFFeedbackBytesGenerator implements MacDerivationFunction {
    ...
    public KDFFeedbackBytesGenerator(Mac mac) {
        ...
        this.f7892h = macSize;
        this.f7894k = new byte[macSize];
    }

    private void generateNext() {
        if (this.generatedBytes == 0) {
            Mac mac = this.prf;
            byte[] bArr = this.f7893iv;
            mac.update(bArr, 0, bArr.length);
        } else {
            Mac mac2 = this.prf;
            byte[] bArr2 = this.f7894k;
            mac2.update(bArr2, 0, bArr2.length);
        }
        if (this.useCounter) {
            int i = (this.generatedBytes / this.f7892h) + 1;
            byte[] bArr3 = this.ios;
        ...
        this.prf.doFinal(this.f7894k, 0);
    }
    ...
    public int generateBytes(byte[] bArr, int i, int i2) throws DataLengthException, IllegalArgumentException {
        ...
        System.arraycopy(this.f7894k, i7, bArr, i, min);
        ...
        while (true) {
            ...
            generateNext();
            min = Math.min(this.f7892h, i8);
            System.arraycopy(this.f7894k, 0, bArr, i, min);
            this.generatedBytes += min;
            i8 -= min;
        }
    }
    ...
    public void init(DerivationParameters derivationParameters) {
        if (derivationParameters instanceof KDFFeedbackParameters) {
            ...
            if (kDFFeedbackParameters.useCounter()) {
                BigInteger multiply = TWO.pow(r).multiply(BigInteger.valueOf((long) this.f7892h));
                if (multiply.compareTo(INTEGER_MAX) != 1) {
                    i = multiply.intValue();
                }
            }
            this.maxSizeExcl = i;
            this.f7893iv = kDFFeedbackParameters.getIV();
