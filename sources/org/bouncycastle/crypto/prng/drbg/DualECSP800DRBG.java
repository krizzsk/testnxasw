...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _P */
    private ECPoint f8060_P;

    /* renamed from: _Q */
    private ECPoint f8061_Q;
    private ECCurve.C3310Fp _curve;
    ...
    /* renamed from: _s */
    private byte[] f8062_s;
    private int _sLength;
    private int _securityStrength;
    private int _seedlen;

    static {
        ECCurve.C3310Fp fp = (ECCurve.C3310Fp) NISTNamedCurves.getByName("P-256").getCurve();
        nistPoints[0] = new DualECPoints(128, fp.createPoint(p256_Px, p256_Py), fp.createPoint(p256_Qx, p256_Qy), 1);
        ECCurve.C3310Fp fp2 = (ECCurve.C3310Fp) NISTNamedCurves.getByName("P-384").getCurve();
        nistPoints[1] = new DualECPoints(192, fp2.createPoint(p384_Px, p384_Py), fp2.createPoint(p384_Qx, p384_Qy), 1);
        ECCurve.C3310Fp fp3 = (ECCurve.C3310Fp) NISTNamedCurves.getByName("P-521").getCurve();
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ...
            while (true) {
                if (i2 == dualECPointsArr.length) {
                    ...
                    this.f8060_P = dualECPointsArr[i2].getP();
                    this.f8061_Q = dualECPointsArr[i2].getQ();
                } else {
                    throw new IllegalArgumentException("Requested security strength is not supported by digest");
                }
            }
            if (this.f8060_P != null) {
                byte[] hash_df = Utils.hash_df(this._digest, concatenate, this._seedlen);
                this.f8062_s = hash_df;
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, 512)) {
            ...
            if (bArr2 != null) {
                bigInteger = new BigInteger(1, xor(this.f8062_s, Utils.hash_df(this._digest, bArr2, this._seedlen)));
            } else {
                bigInteger = new BigInteger(1, this.f8062_s);
            }
            int i = 0;
            Arrays.fill(bArr, (byte) 0);
            int i2 = 0;
            for (int i3 = 0; i3 < length2; i3++) {
                bigInteger = getScalarMultipleXCoord(this.f8060_P, bigInteger);
                byte[] byteArray = getScalarMultipleXCoord(this.f8061_Q, bigInteger).toByteArray();
            ...
            if (i2 < bArr.length) {
                bigInteger = getScalarMultipleXCoord(this.f8060_P, bigInteger);
                byte[] byteArray2 = getScalarMultipleXCoord(this.f8061_Q, bigInteger).toByteArray();
            ...
            this.f8062_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f8060_P, bigInteger));
            return length;
        }
    }
    ...
    public void reseed(byte[] bArr) {
        if (!Utils.isTooLarge(bArr, 512)) {
            this.f8062_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f8062_s, this._seedlen), getEntropy(), bArr), this._seedlen);
            this._reseedCounter = 0;
            return;
        }
