...
public class Mat {
    ...
    public Mat(Mat mat, Rect rect) {
        this.nativeObj = n_Mat(mat.nativeObj, rect.f8527y, rect.height + rect.f8527y, rect.f8526x, rect.f8526x + rect.width);
    }
    ...
    public void locateROI(Size size, Point point) {
        ...
        if (point != null) {
            point.f8521x = dArr2[0];
            point.f8522y = dArr2[1];
        }
    }
    ...
    public Mat submat(Rect rect) {
        return new Mat(n_submat(this.nativeObj, rect.f8526x, rect.f8527y, rect.width, rect.height));
    }
