...
import p218io.flutter.embedding.engine.dart.PlatformMessageHandler;
...
public class FlutterJNI {
    ...
    public void loadLibrary() {
        if (loadLibraryCalled) {
            Log.m45261w(TAG, "FlutterJNI.loadLibrary called more than once");
        }
    ...
    public void prefetchDefaultFontManager() {
        if (prefetchDefaultFontManagerCalled) {
            Log.m45261w(TAG, "FlutterJNI.prefetchDefaultFontManager called more than once");
        }
    ...
    public void init(Context context, String[] strArr, String str, String str2, String str3, long j) {
        if (initCalled) {
            Log.m45261w(TAG, "FlutterJNI.init called more than once");
        }
    ...
    public static Bitmap decodeImage(ByteBuffer byteBuffer, long j) {
        if (Build.VERSION.SDK_INT >= 28) {
            try {
                return ImageDecoder.decodeBitmap(ImageDecoder.createSource(byteBuffer), new ImageDecoder.OnHeaderDecodedListener(j) {
                ...
                Log.m45256e(TAG, "Failed to decode image", e);
            }
        }
        return null;
    }
    ...
    public void dispatchEmptyPlatformMessage(String str, int i) {
        ...
        Log.m45261w(TAG, "Tried to send a platform message to Flutter, but FlutterJNI was detached from native C++. Could not send. Channel: " + str + ". Response ID: " + i);
    }
    ...
    public void dispatchPlatformMessage(String str, ByteBuffer byteBuffer, int i, int i2) {
        ...
        Log.m45261w(TAG, "Tried to send a platform message to Flutter, but FlutterJNI was detached from native C++. Could not send. Channel: " + str + ". Response ID: " + i2);
    }
    ...
    public void invokePlatformMessageEmptyResponseCallback(int i) {
        ...
        try {
            if (isAttached()) {
                ...
                Log.m45261w(TAG, "Tried to send a platform message response, but FlutterJNI was detached from native C++. Could not send. Response ID: " + i);
            }
        } finally {
    ...
    public void requestDartDeferredLibrary(int i) {
        ...
        if (deferredComponentManager2 != null) {
            ...
            Log.m45255e(TAG, "No DeferredComponentManager found. Android setup must be completed before using split AOT deferred components.");
        }
    }
