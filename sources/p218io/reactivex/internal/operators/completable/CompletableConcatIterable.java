...
public final class CompletableConcatIterable extends Completable {
    ...
    /* renamed from: a */
    final Iterable<? extends CompletableSource> f60864a;

    public CompletableConcatIterable(Iterable<? extends CompletableSource> iterable) {
        this.f60864a = iterable;
    }

    public void subscribeActual(CompletableObserver completableObserver) {
        try {
            ConcatInnerObserver concatInnerObserver = new ConcatInnerObserver(completableObserver, (Iterator) ObjectHelper.requireNonNull(this.f60864a.iterator(), "The iterator returned is null"));
            completableObserver.onSubscribe(concatInnerObserver.f60865sd);
    ...
    static final class ConcatInnerObserver extends AtomicInteger implements CompletableObserver {
        ...
        /* renamed from: sd */
        final SequentialDisposable f60865sd = new SequentialDisposable();
        ...
        public void onSubscribe(Disposable disposable) {
            this.f60865sd.replace(disposable);
        }
        ...
        public void next() {
            if (!this.f60865sd.isDisposed() && getAndIncrement() == 0) {
                Iterator<? extends CompletableSource> it = this.sources;
                while (!this.f60865sd.isDisposed()) {
                    try {
                        if (!it.hasNext()) {
