...
public final class ObservableFromIterable<T> extends Observable<T> {
    ...
    /* renamed from: a */
    final Iterable<? extends T> f61600a;

    public ObservableFromIterable(Iterable<? extends T> iterable) {
        this.f61600a = iterable;
    }

    public void subscribeActual(Observer<? super T> observer) {
        try {
            Iterator<? extends T> it = this.f61600a.iterator();
            try {
                if (!it.hasNext()) {
    ...
    static final class FromIterableDisposable<T> extends BasicQueueDisposable<T> {
        ...
        /* renamed from: it */
        final Iterator<? extends T> f61601it;
        ...
        public void run() {
            while (!isDisposed()) {
                try {
                    this.downstream.onNext(ObjectHelper.requireNonNull(this.f61601it.next(), "The iterator returned a null value"));
                    if (!isDisposed()) {
                        try {
                            if (!this.f61601it.hasNext()) {
                                if (!isDisposed()) {
        ...
        public T poll() {
            ...
            if (!this.checkNext) {
                this.checkNext = true;
            } else if (!this.f61601it.hasNext()) {
                this.done = true;
                return null;
            }
            return ObjectHelper.requireNonNull(this.f61601it.next(), "The iterator returned a null value");
        }
