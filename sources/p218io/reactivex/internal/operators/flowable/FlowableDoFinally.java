...
public final class FlowableDoFinally<T> extends C22212a<T, T> {

    /* renamed from: a */
    final Action f61040a;
    ...
    public void subscribeActual(Subscriber<? super T> subscriber) {
        if (subscriber instanceof ConditionalSubscriber) {
            this.source.subscribe(new DoFinallyConditionalSubscriber((ConditionalSubscriber) subscriber, this.f61040a));
        } else {
            this.source.subscribe(new DoFinallySubscriber(subscriber, this.f61040a));
        }
    }
    ...
    static final class DoFinallySubscriber<T> extends BasicIntQueueSubscription<T> implements FlowableSubscriber<T> {
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.upstream, subscription)) {
                ...
                if (subscription instanceof QueueSubscription) {
                    this.f61042qs = (QueueSubscription) subscription;
                }
        ...
        public int requestFusion(int i) {
            QueueSubscription<T> queueSubscription = this.f61042qs;
            boolean z = false;
        ...
        public boolean isEmpty() {
            return this.f61042qs.isEmpty();
        }

        public T poll() throws Exception {
            T poll = this.f61042qs.poll();
    ...
    static final class DoFinallyConditionalSubscriber<T> extends BasicIntQueueSubscription<T> implements ConditionalSubscriber<T> {
        ...
        public void onSubscribe(Subscription subscription) {
            if (SubscriptionHelper.validate(this.upstream, subscription)) {
                this.upstream = subscription;
                if (subscription instanceof QueueSubscription) {
                    this.f61041qs = (QueueSubscription) subscription;
                }
                this.downstream.onSubscribe(this);
            }
        }
        ...
        public int requestFusion(int i) {
            QueueSubscription<T> queueSubscription = this.f61041qs;
            boolean z = false;
            if (queueSubscription == null || (i & 4) != 0) {
                return 0;
            }
        ...
        public boolean isEmpty() {
            return this.f61041qs.isEmpty();
        }

        public T poll() throws Exception {
            T poll = this.f61041qs.poll();
            if (poll == null && this.syncFused) {
                runFinally();
            }
            return poll;
