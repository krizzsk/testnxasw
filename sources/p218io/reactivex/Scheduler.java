...
import p218io.reactivex.internal.schedulers.NewThreadWorker;
import p218io.reactivex.internal.schedulers.SchedulerWhen;
...
public abstract class Scheduler {
    ...
    /* renamed from: a */
    static final long f60790a = TimeUnit.MINUTES.toNanos(Long.getLong("rx2.scheduler.drift-tolerance", 15).longValue());
    ...
    public static abstract class Worker implements Disposable {
        ...
        final class PeriodicTask implements SchedulerRunnableIntrospection, Runnable {
            ...
            public void run() {
                ...
                if (!this.f60792sd.isDisposed()) {
                    long now = Worker.this.now(TimeUnit.NANOSECONDS);
                    long j2 = this.lastNowNanoseconds;
                    if (Scheduler.f60790a + now < j2 || now >= j2 + this.periodInNanoseconds + Scheduler.f60790a) {
                    ...
                    this.f60792sd.replace(Worker.this.schedule(this, j - now, TimeUnit.NANOSECONDS));
                }
            }
    ...
    static final class DisposeTask implements Disposable, SchedulerRunnableIntrospection, Runnable {
        ...
        public void dispose() {
            if (this.runner == Thread.currentThread()) {
                Worker worker = this.f60791w;
                if (worker instanceof NewThreadWorker) {
                    ((NewThreadWorker) worker).shutdown();
                    return;
                }
            }
            this.f60791w.dispose();
        }

        public boolean isDisposed() {
            return this.f60791w.isDisposed();
        }
