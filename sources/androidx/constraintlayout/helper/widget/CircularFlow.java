...
public class CircularFlow extends VirtualLayout {
    ...
    public void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            ...
            for (int i = 0; i < indexCount; i++) {
                ...
                if (index == C0273R.styleable.ConstraintLayout_Layout_circularflow_viewCenter) {
                    this.mViewCenter = obtainStyledAttributes.getResourceId(index, 0);
                } else if (index == C0273R.styleable.ConstraintLayout_Layout_circularflow_angles) {
                    String string = obtainStyledAttributes.getString(index);
                    this.mReferenceAngles = string;
                    setAngles(string);
                } else if (index == C0273R.styleable.ConstraintLayout_Layout_circularflow_radiusInDP) {
                    String string2 = obtainStyledAttributes.getString(index);
                    this.mReferenceRadius = string2;
                    setRadius(string2);
                } else if (index == C0273R.styleable.ConstraintLayout_Layout_circularflow_defaultAngle) {
                    Float valueOf = Float.valueOf(obtainStyledAttributes.getFloat(index, DEFAULT_ANGLE));
                    this.mReferenceDefaultAngle = valueOf;
                    setDefaultAngle(valueOf.floatValue());
                } else if (index == C0273R.styleable.ConstraintLayout_Layout_circularflow_defaultRadius) {
            ...
            if (viewById != null) {
                ...
                if (iArr == null || i >= iArr.length) {
                    ...
                    if (num == null || num.intValue() == -1) {
                        SystemUtils.log(6, TAG, "Added radius to view with id: " + ((String) this.mMap.get(Integer.valueOf(viewById.getId()))), (Throwable) null, "androidx.constraintlayout.helper.widget.CircularFlow", 201);
                    } else {
                        this.mCountRadius++;
                ...
                if (fArr == null || i >= fArr.length) {
                    ...
                    if (f2 == null || f2.floatValue() == -1.0f) {
                        SystemUtils.log(6, TAG, "Added angle to view with id: " + ((String) this.mMap.get(Integer.valueOf(viewById.getId()))), (Throwable) null, "androidx.constraintlayout.helper.widget.CircularFlow", 214);
                    } else {
                        this.mCountAngle++;
    ...
    public void updateRadius(View view, int i) {
        if (!isUpdatable(view)) {
            SystemUtils.log(6, TAG, "It was not possible to update radius to view with id: " + view.getId(), (Throwable) null, "androidx.constraintlayout.helper.widget.CircularFlow", 257);
            return;
        }
    ...
    public void updateAngle(View view, float f) {
        if (!isUpdatable(view)) {
            SystemUtils.log(6, TAG, "It was not possible to update angle to view with id: " + view.getId(), (Throwable) null, "androidx.constraintlayout.helper.widget.CircularFlow", 278);
            return;
        }
        int indexFromId = indexFromId(view.getId());
    ...
    public void updateReference(View view, int i, float f) {
        if (!isUpdatable(view)) {
            SystemUtils.log(6, TAG, "It was not possible to update radius and angle to view with id: " + view.getId(), (Throwable) null, "androidx.constraintlayout.helper.widget.CircularFlow", 300);
            return;
        }
        int indexFromId = indexFromId(view.getId());
