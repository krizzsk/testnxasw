...
public class ConstraintLayoutStates {
    ...
    public void updateConstraints(int i, float f, float f2) {
        ...
        if (constraintSet == null) {
            SystemUtils.log(2, "ConstraintLayoutStates", "NO Constraint set found ! id=" + i + ", dim =" + f + ", " + f2, (Throwable) null, "androidx.constraintlayout.widget.ConstraintLayoutStates", 117);
            return;
        }
    ...
    static class State {
        ...
        public State(Context context, XmlPullParser xmlPullParser) {
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), C0273R.styleable.State);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0273R.styleable.State_android_id) {
                    this.mId = obtainStyledAttributes.getResourceId(index, this.mId);
                } else if (index == C0273R.styleable.State_constraints) {
    ...
    static class Variant {
        ...
        public Variant(Context context, XmlPullParser xmlPullParser) {
            TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), C0273R.styleable.Variant);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0273R.styleable.Variant_constraints) {
                    this.mConstraintID = obtainStyledAttributes.getResourceId(index, this.mConstraintID);
                    String resourceTypeName = context.getResources().getResourceTypeName(this.mConstraintID);
                    context.getResources().getResourceName(this.mConstraintID);
                    if ("layout".equals(resourceTypeName)) {
                ...
                } else if (index == C0273R.styleable.Variant_region_heightLessThan) {
                    this.mMaxHeight = obtainStyledAttributes.getDimension(index, this.mMaxHeight);
                } else if (index == C0273R.styleable.Variant_region_heightMoreThan) {
                    this.mMinHeight = obtainStyledAttributes.getDimension(index, this.mMinHeight);
                } else if (index == C0273R.styleable.Variant_region_widthLessThan) {
                    this.mMaxWidth = obtainStyledAttributes.getDimension(index, this.mMaxWidth);
                } else if (index == C0273R.styleable.Variant_region_widthMoreThan) {
                    this.mMinWidth = obtainStyledAttributes.getDimension(index, this.mMinWidth);
                } else {
                    SystemUtils.log(2, "ConstraintLayoutStates", "Unknown tag", (Throwable) null, "androidx.constraintlayout.widget.ConstraintLayoutStates$Variant", 232);
                }
            }
            obtainStyledAttributes.recycle();
        }
            ...
            if (attributeName == null || attributeValue == null || !"id".equals(attributeName)) {
                ...
                if (i == -1) {
                    if (attributeValue.length() > 1) {
                        ...
                        SystemUtils.log(6, "ConstraintLayoutStates", "error in parsing id", (Throwable) null, "androidx.constraintlayout.widget.ConstraintLayoutStates", 366);
                    }
                }
