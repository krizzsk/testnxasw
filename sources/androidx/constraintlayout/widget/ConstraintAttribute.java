...
public class ConstraintAttribute {
    ...
    public boolean isContinuous() {
        int i = C02711.f107x66adad53[this.mType.ordinal()];
    ...
    public void setValue(float[] fArr) {
        boolean z = false;
        switch (C02711.f107x66adad53[this.mType.ordinal()]) {
            case 1:
            case 6:
    ...
    public static void setAttributes(View view, HashMap<String, ConstraintAttribute> hashMap) {
        ...
        for (String next : hashMap.keySet()) {
            ...
            if (!constraintAttribute.mMethod) {
                str = GuideParamModel.ACTION_SET + next;
            } else {
                str = next;
            }
            try {
                switch (C02711.f107x66adad53[constraintAttribute.mType.ordinal()]) {
                    case 1:
                ...
                SystemUtils.log(6, TAG, e.getMessage(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 373);
                SystemUtils.log(6, TAG, " Custom Attribute \"" + next + "\" not found on " + cls.getName(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 374);
                StringBuilder sb = new StringBuilder();
                sb.append(cls.getName());
                sb.append(" must have a method ");
                sb.append(str);
                SystemUtils.log(6, TAG, sb.toString(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 375);
            } catch (IllegalAccessException e2) {
                SystemUtils.log(6, TAG, " Custom Attribute \"" + next + "\" not found on " + cls.getName(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 377);
                e2.printStackTrace();
            } catch (InvocationTargetException e3) {
                SystemUtils.log(6, TAG, " Custom Attribute \"" + next + "\" not found on " + cls.getName(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 380);
                e3.printStackTrace();
            }
        }
    }
    ...
    public void applyCustom(View view) {
        ...
        if (!this.mMethod) {
            str = GuideParamModel.ACTION_SET + str2;
        } else {
            str = str2;
        }
        try {
            ...
            SystemUtils.log(6, TAG, e.getMessage(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 429);
            SystemUtils.log(6, TAG, " Custom Attribute \"" + str2 + "\" not found on " + cls.getName(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 430);
            StringBuilder sb = new StringBuilder();
            sb.append(cls.getName());
            sb.append(" must have a method ");
            sb.append(str);
            SystemUtils.log(6, TAG, sb.toString(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 431);
        } catch (IllegalAccessException e2) {
            SystemUtils.log(6, TAG, " Custom Attribute \"" + str2 + "\" not found on " + cls.getName(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 433);
            e2.printStackTrace();
        } catch (InvocationTargetException e3) {
            SystemUtils.log(6, TAG, " Custom Attribute \"" + str2 + "\" not found on " + cls.getName(), (Throwable) null, "androidx.constraintlayout.widget.ConstraintAttribute", 436);
            e3.printStackTrace();
        }
    }
    ...
    public static void parse(Context context, XmlPullParser xmlPullParser, HashMap<String, ConstraintAttribute> hashMap) {
        ...
        Object valueOf;
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(Xml.asAttributeSet(xmlPullParser), C0273R.styleable.CustomAttribute);
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (index == C0273R.styleable.CustomAttribute_attributeName) {
                str = obtainStyledAttributes.getString(index);
                if (str != null && str.length() > 0) {
                    str = Character.toUpperCase(str.charAt(0)) + str.substring(1);
                }
            } else if (index == C0273R.styleable.CustomAttribute_methodName) {
                str = obtainStyledAttributes.getString(index);
                z = true;
            } else if (index == C0273R.styleable.CustomAttribute_customBoolean) {
                obj = Boolean.valueOf(obtainStyledAttributes.getBoolean(index, false));
                attributeType2 = AttributeType.BOOLEAN_TYPE;
            } else {
                if (index == C0273R.styleable.CustomAttribute_customColorValue) {
                    attributeType = AttributeType.COLOR_TYPE;
                    valueOf = Integer.valueOf(obtainStyledAttributes.getColor(index, 0));
                } else if (index == C0273R.styleable.CustomAttribute_customColorDrawableValue) {
                    attributeType = AttributeType.COLOR_DRAWABLE_TYPE;
                    valueOf = Integer.valueOf(obtainStyledAttributes.getColor(index, 0));
                } else if (index == C0273R.styleable.CustomAttribute_customPixelDimension) {
                    attributeType = AttributeType.DIMENSION_TYPE;
                    valueOf = Float.valueOf(TypedValue.applyDimension(1, obtainStyledAttributes.getDimension(index, 0.0f), context.getResources().getDisplayMetrics()));
                } else if (index == C0273R.styleable.CustomAttribute_customDimension) {
                    attributeType = AttributeType.DIMENSION_TYPE;
                    valueOf = Float.valueOf(obtainStyledAttributes.getDimension(index, 0.0f));
                } else if (index == C0273R.styleable.CustomAttribute_customFloatValue) {
                    attributeType = AttributeType.FLOAT_TYPE;
                    valueOf = Float.valueOf(obtainStyledAttributes.getFloat(index, Float.NaN));
                } else if (index == C0273R.styleable.CustomAttribute_customIntegerValue) {
                    attributeType = AttributeType.INT_TYPE;
                    valueOf = Integer.valueOf(obtainStyledAttributes.getInteger(index, -1));
                } else if (index == C0273R.styleable.CustomAttribute_customStringValue) {
                    attributeType = AttributeType.STRING_TYPE;
                    valueOf = obtainStyledAttributes.getString(index);
                } else if (index == C0273R.styleable.CustomAttribute_customReference) {
