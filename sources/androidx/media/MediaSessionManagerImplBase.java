...
class MediaSessionManagerImplBase implements MediaSessionManager.MediaSessionManagerImpl {
    ...
    public boolean isTrustedForMediaControl(MediaSessionManager.RemoteUserInfoImpl remoteUserInfoImpl) {
        try {
            if (SystemUtils.getApplicationInfo(this.mContext.getPackageManager(), remoteUserInfoImpl.getPackageName(), 0).uid != remoteUserInfoImpl.getUid()) {
                if (DEBUG) {
                    SystemUtils.log(3, TAG, "Package name " + remoteUserInfoImpl.getPackageName() + " doesn't match with the uid " + remoteUserInfoImpl.getUid(), (Throwable) null, "androidx.media.MediaSessionManagerImplBase", 71);
                }
                return false;
            ...
            if (DEBUG) {
                SystemUtils.log(3, TAG, "Package " + remoteUserInfoImpl.getPackageName() + " doesn't exist", (Throwable) null, "androidx.media.MediaSessionManagerImplBase", 64);
            }
            return false;
        }
    }
