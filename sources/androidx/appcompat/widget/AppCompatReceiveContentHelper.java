    ...
    static boolean maybeHandleDragEventViaPerformReceiveContent(View view, DragEvent dragEvent) {
        if (Build.VERSION.SDK_INT >= 24 && dragEvent.getLocalState() == null && ViewCompat.getOnReceiveContentMimeTypes(view) != null) {
            ...
            if (tryGetActivity == null) {
                SystemUtils.log(4, LOG_TAG, "Can't handle drop: no activity: view=" + view, (Throwable) null, "androidx.appcompat.widget.AppCompatReceiveContentHelper", 100);
                return false;
            ...
            public boolean onCommitContent(InputContentInfoCompat inputContentInfoCompat, int i, Bundle bundle) {
                if (Build.VERSION.SDK_INT >= 25 && (i & 1) != 0) {
                    try {
                        ...
                        SystemUtils.log(5, AppCompatReceiveContentHelper.LOG_TAG, "Can't insert content from IME; requestPermission() failed", e, "androidx.appcompat.widget.AppCompatReceiveContentHelper$1", 189);
                        return false;
                    }
                }
                if (ViewCompat.performReceiveContent(View.this, new ContentInfoCompat.Builder(new ClipData(inputContentInfoCompat.getDescription(), new ClipData.Item(inputContentInfoCompat.getContentUri())), 2).setLinkUri(inputContentInfoCompat.getLinkUri()).setExtras(bundle).build()) == null) {
                    return true;
                }
                return false;
            }
