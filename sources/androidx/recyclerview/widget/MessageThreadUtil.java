    ...
    public ThreadUtil.MainThreadCallback<T> getMainThreadProxy(final ThreadUtil.MainThreadCallback<T> mainThreadCallback) {
        return new ThreadUtil.MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem next = C06731.this.mQueue.next();
                    while (next != null) {
                        int i = next.what;
                        if (i == 1) {
                            ...
                            SystemUtils.log(6, "ThreadUtil", "Unsupported message, what=" + next.what, (Throwable) null, "androidx.recyclerview.widget.MessageThreadUtil$1$1", 76);
                        } else {
                            mainThreadCallback.removeTile(next.arg1, next.arg2);
                        }
                        next = C06731.this.mQueue.next();
                    }
                }
            };
    ...
    public ThreadUtil.BackgroundCallback<T> getBackgroundProxy(final ThreadUtil.BackgroundCallback<T> backgroundCallback) {
        return new ThreadUtil.BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem next = C06752.this.mQueue.next();
                        if (next == null) {
                        ...
                        int i = next.what;
                        if (i == 1) {
                            C06752.this.mQueue.removeMessages(1);
                            backgroundCallback.refresh(next.arg1);
                        } else if (i == 2) {
                            C06752.this.mQueue.removeMessages(2);
                            C06752.this.mQueue.removeMessages(3);
                            ...
                            SystemUtils.log(6, "ThreadUtil", "Unsupported message, what=" + next.what, (Throwable) null, "androidx.recyclerview.widget.MessageThreadUtil$2$1", 162);
                        } else {
