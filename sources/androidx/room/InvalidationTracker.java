...
public class InvalidationTracker {
        ...
        this.mRefreshRunnable = new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    try {
                        SystemUtils.log(6, "ROOM", "Cannot run invalidation tracker. Is the db closed?", e, "androidx.room.InvalidationTracker$1", 398);
                    } catch (Throwable th) {
                        closeLock.unlock();
                        throw th;
                    }
    ...
    public void internalInit(SupportSQLiteDatabase supportSQLiteDatabase) {
        synchronized (this) {
            if (this.mInitialized) {
                SystemUtils.log(6, "ROOM", "Invalidation tracker is initialized twice :/.", (Throwable) null, "androidx.room.InvalidationTracker", 170);
                return;
            }
    ...
    public boolean ensureInitialization() {
        ...
        SystemUtils.log(6, "ROOM", "database is not initialized even though it is open", (Throwable) null, "androidx.room.InvalidationTracker", 351);
        return false;
    }
        ...
        if (!supportSQLiteDatabase.inTransaction()) {
            while (true) {
                try {
                    ...
                    SystemUtils.log(6, "ROOM", "Cannot run invalidation tracker. Is the db closed?", e, "androidx.room.InvalidationTracker", 521);
                    return;
                }
            }
        }
