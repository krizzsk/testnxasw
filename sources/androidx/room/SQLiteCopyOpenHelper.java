                    ...
                    if (readVersion == this.mDatabaseVersion) {
                        ...
                        if (this.mContext.deleteDatabase(databaseName)) {
                            try {
                                ...
                                SystemUtils.log(5, "ROOM", "Unable to copy database file.", e, "androidx.room.SQLiteCopyOpenHelper", 164);
                            }
                        } else {
                            SystemUtils.log(5, "ROOM", "Failed to delete database file (" + databaseName + ") for a copy destructive migration.", (Throwable) null, "androidx.room.SQLiteCopyOpenHelper", 167);
                        }
                        copyLock.unlock();
                    }
                } catch (IOException e2) {
                    SystemUtils.log(5, "ROOM", "Unable to read database version.", e2, "androidx.room.SQLiteCopyOpenHelper", 144);
                    copyLock.unlock();
                }
            }
