...
public class DiagnosticsWorker extends Worker {
    ...
    private static String workSpecRows(WorkNameDao workNameDao, WorkTagDao workTagDao, SystemIdInfoDao systemIdInfoDao, List<WorkSpec> list) {
        ...
        for (WorkSpec next : list) {
            Integer num = null;
            SystemIdInfo systemIdInfo = systemIdInfoDao.getSystemIdInfo(next.f149id);
            if (systemIdInfo != null) {
                num = Integer.valueOf(systemIdInfo.systemId);
            }
            sb.append(workSpecRow(next, TextUtils.join(",", workNameDao.getNamesForWorkSpecId(next.f149id)), num, TextUtils.join(",", workTagDao.getTagsForWorkSpecId(next.f149id))));
        }
        return sb.toString();
    }

    private static String workSpecRow(WorkSpec workSpec, String str, Integer num, String str2) {
        return String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", new Object[]{workSpec.f149id, workSpec.workerClassName, num, workSpec.state.name(), str, str2});
    }
}
