...
public final class NewCapturedTypeKt {
    public static final SimpleType captureFromArguments(SimpleType simpleType, CaptureStatus captureStatus) {
        ...
        List<TypeProjection> a = m5256a(unwrappedType, captureStatus);
        if (a == null) {
            return null;
        }
        return m5257a(unwrappedType, (List<? extends TypeProjection>) a);
    }

    /* renamed from: a */
    private static final SimpleType m5257a(UnwrappedType unwrappedType, List<? extends TypeProjection> list) {
    ...
    /* renamed from: a */
    private static final List<TypeProjection> m5256a(UnwrappedType unwrappedType, CaptureStatus captureStatus) {
            ...
            while (true) {
                ...
                if (typeProjection2.getProjectionKind() != Variance.INVARIANT) {
                    ...
                    for (KotlinType safeSubstitute : upperBounds) {
                        arrayList2.add(KotlinTypePreparator.Default.INSTANCE.prepareType(buildSubstitutor.safeSubstitute(safeSubstitute, Variance.INVARIANT).unwrap()));
                    }
                    List list2 = (List) arrayList2;
                    if (!typeProjection2.isStarProjection() && typeProjection2.getProjectionKind() == Variance.OUT_VARIANCE) {
                        list2.add(KotlinTypePreparator.Default.INSTANCE.prepareType(typeProjection2.getType().unwrap()));
                    }
